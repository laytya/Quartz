...
local UnitGUID = function(unit)
     local _, exists, guid = pcall(UnitExists, unit)
    if exists then return guid end
end

local GetSpellInfo = SpellInfo
...
f:SetScript("OnEvent", function()
    return this[event](event, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8, arg9, arg10)
end)
...
function f:UNIT_CASTEVENT(srcGUID, dstGUID, eventType, spellID, duration)
    if eventType == "START" then
        CastStart(srcGUID, "CAST",  spellID, duration)
	elseif eventType == "CHANNEL" then
        CastStart(srcGUID, "CHANNEL",  spellID, duration)
	elseif eventType == "FAIL" then
		CastStop(srcGUID, spellID, "FAILED", "STOP")
...
f.SPELLCAST_CHANNEL_UPDATE = function()
...
FireToUnits = function(event, guid, ...)
    if playerGUID == guid then
        callbacks:Fire(event, getn(arg)+1, "player", unpack(arg))
    end
    for _, unit in ipairs(commonUnits) do
        if UnitGUID(unit) == guid then
            callbacks:Fire(event, getn(arg)+1, unit, unpack(arg))
        end
    end

    local partyUnit = partyGUIDtoUnit[guid]
    if partyUnit then
        callbacks:Fire(event,getn(arg)+1, partyUnit, unpack(arg))
    end

    local raidUnit = raidGUIDtoUnit[guid]
    if raidUnit then
        callbacks:Fire(event,getn(arg)+1, raidUnit, unpack(arg))
    end
--[[
